...
const CURVE = [
  [0,1.5],[1,1.5],[2,1.5],[3,1.5],[4,1.5],[5,1.5],[6,2],
  [7,3],[8,6],[9,12],[10,8],[11,6],[12,10],[13,20],[14,15],
  [15,10],[16,8],[17,10],[18,18],[19,28],[20,38],[21,32],[22,20],[23,10],[24,3]
];
...
export function generateDay(opts = {}) {
  const {
    ...
    demandScale    = 1.0,
    curve          = CURVE,
    captains       = { mN:8, mIn:7, mOut:15, eN:6, eIn:15, eOut:23, overnight:12 },
    packers        = { mN:6, mIn:8, mOut:16, eN:8, eIn:16, eOut:24, overnight:10 },
  ...
  while (t < 24) {
    ...
    returnQueue = returnQueue.filter(r => {
      if (r.returnAt <= t) { capsOnRoad = Math.max(0, capsOnRoad - r.n); return false; }
      return true;
    });
    ...
    const assignSec  = avgAssignSec * (0.6 + rand() * 0.8) * (1 + loadFactor * 1.5);
    ...
    const zone     = `zone-${1 + Math.floor(rand() * 4)}`;
...
export function slotSummaries(orders, slotMin = 15) {
  ...
  for (let h = 0; h < 24; h += step) {
    ...
    const delivered = orders.filter(o => o.deliveredAt >= h && o.deliveredAt < end);
    const pending   = orders.filter(o => o.createdAt < end && o.deliveredAt >= end);
    const breached  = arrived.filter(o => o.slaBreached);
    const dispatched = orders.filter(o => o.dispatchedAt >= h && o.dispatchedAt < end);

    const oldestAge = pending.length > 0
      ? Math.round((end - Math.min(...pending.map(o => o.createdAt))) * 60)
      : 0;

    const avgAssign = arrived.length > 0
      ? Math.round(arrived.reduce((s, o) => s + o.assignLatency, 0) / arrived.length)
      : 0;
    const avgPrep   = arrived.length > 0
      ? +(arrived.reduce((s, o) => s + o.prepTime, 0) / arrived.length).toFixed(1)
      : 0;
    const avgTransit = delivered.length > 0
      ? +(delivered.reduce((s, o) => s + o.transitTime, 0) / delivered.length).toFixed(1)
      : 0;
    const avgRoundTrip = delivered.length > 0
      ? +(delivered.reduce((s, o) => s + o.roundTrip, 0) / delivered.length).toFixed(1)
      : 0;
    ...
    const stackDepth = dispatched.length > 0
    ...
    slots.push({
      ...
      slaRate:       arrived.length > 0 ? +(breached.length / arrived.length).toFixed(3) : 0,
      oldestAge,                        // minutes
...
function dayStats(orders) {
  ...
  const avgTotal      = +(orders.reduce((s,o) => s + o.totalMinutes, 0) / totalOrders).toFixed(1);
  const p95Total      = +orders.map(o => o.totalMinutes).sort((a,b) => a-b)[Math.floor(totalOrders * .95)].toFixed(1);
  const peakSlotOrders = Math.max(...slotSummaries(orders, 15).map(s => s.incoming));
  return { totalOrders, totalBreached, slaBreachRate: +(totalBreached/totalOrders).toFixed(3), avgTotal, p95Total, peakSlotOrders };
}
